/* injector-iframe.js
   Вставляет iframe с ad2.html после последнего абзаца.
   Источники: GitHub Pages → jsDelivr → raw.githubusercontent; srcdoc-фолбэк.
   Работает в Wix (SPA), лениво запускается (IntersectionObserver). */

(function(){
  // === НАСТРОЙКИ ===
  var BLOCK_ID = 'R-A-14383531-11';        // ваш RTB-блок (в кабинете Яндекса — 300×250)
  ...
  function lastTextBlock(root){
    var nodes=[].slice.call(root.querySelectorAll('p, li, blockquote, .wixui-rich-text__text, [data-testid="richTextElement"]'));
  ...
  function placeAnchor(){
    var root=contentRoot(), last=lastTextBlock(root);
    if(!last){ log('[ya-rtb] last paragraph not found'); return null; }
    var wrap=buildAnchor(); if(!wrap.parentNode) last.insertAdjacentElement('afterend', wrap);
    return wrap;
  }

  // --- HTML для srcdoc-фолбэка (если все источники недоступны) ---
  function buildSrcdoc(){
    var id='ya_rtb_root';
    var html='<!doctype html><html><head><meta charset="utf-8">'
      +'<meta name="referrer" content="no-referrer"><style>html,body{margin:0;padding:0;background:transparent}#'+id+'{display:block;margin:0 auto;max-width:100%}</style>'
      +'</head><body><div id="'+id+'" style="width:'+SIZE.w+'px;height:'+SIZE.h+'px"></div>'
  ...
  function mountIframe(wrap){
    ...
    function tryNext(){
      ...
      if(current<SOURCES.length){
        ...
        iframe.src=u;
        clearTimeout(timer);
        timer=setTimeout(function(){ log('[ya-rtb] timeout, switch'); tryNext(); }, TIMEOUT_MS);
      }else{
        log('[ya-rtb] use srcdoc fallback');
  ...
  function init(){
    ...
    // ленивый старт, чтобы не дергать сетку слишком рано
  ...
  ready(function(){
    ...
    // SPA Wix — повторная вставка при динамике
